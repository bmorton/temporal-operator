...
func TestIsSupported(t *testing.T) {
	if !IsSupported("1.31.2") {
		t.Errorf("expected 1.31.2 to be supported")
	}
...
func TestDefaultUIVersion(t *testing.T) {
	if got := DefaultUIVersion("1.31.2"); got == "" {
		t.Errorf("expected a default UI version for 1.31.2")
	}
...
func TestSupportedVersionsAndGet(t *testing.T) {
	...
	if _, ok := Get("1.31.2"); !ok {
		t.Errorf("expected Get(1.31.2) to return info")
	}
	if _, ok := Get("nope"); ok {
